...
[aligned_traces,aligned_gradS] = Align_templateTraces(interp_c2,'SampleRate=150','SlopeDuration=0.4');
...
for ii=1:num_traces
    subplot(num_rows,num_rows,ii)
    plot(aligned_traces(ii,:),'k')
    hold on
    plot(aligned_gradS(ii,:),'g')
    plot([0,800],[1,1],'--b')
    ylim([-2,5])

    if double_cross(ii) || late_cross(ii)==1
...
for ii=search_start_point
    ...
    try
        ...
        a3store{ii-first_search_idx+1} = a2;
